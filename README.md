...
設定の「開発用ベンチマーク」は、ローカルWhisperモデルの処理速度や端末負荷を確認するための測定ツールです。通常の録音、文字起こし、議事録作成には使用しません。

1. `ggml`形式のWhisperモデル（`.bin`）を選択します。
2. 16kHz・16bit・モノラルのWAV音声を選択します。
3. スレッド数と連続実行回数を指定し、「ベンチマーク実行」を押します。
4. 完了後、「結果CSVを共有」から測定結果を書き出せます。

CSVには端末のメーカー・機種名、Androidバージョン、モデル名と容量、音声ファイル名と長さ、スレッド数、処理時間、RTF、ピークメモリ、温度状態、実行前後のバッテリー残量が記録されます。共有する前に、端末情報やファイル名を外部へ渡して問題がないか確認してください。

RTFは「処理時間 ÷ 音声時間」です。`1.0`未満なら音声の実時間より速く、値が小さいほど高速です。測定値は端末の温度、充電状態、バックグラウンド処理、モデル、音声、スレッド数によって変動します。比較するときは同じ条件で複数回実行してください。この機能は処理性能を測るもので、文字起こし精度を採点するものではありません。
...
パスフレーズと回復キーの両方を失うと、開発者を含め誰も復元できません。APIキーはバックアップに含まれません。
